...
import { 
    startLoading, stopLoading, jsonLoader, renderProjects, fillTable
...
async function addItems(value) { 
    timeOut = setTimeout( async() => { 
        const response = await jsonLoader('search_lake', { 
            projectName: getState().currentProject, name: value 
        }); 
...
function updateManager() { 
    ...
    document.querySelectorAll('input[type="radio"]').forEach(obj => { 
        obj.addEventListener('change', () => { 
            ...
            const tbody = lakeTable().querySelector("tbody"); 
            tbody.innerHTML = ''; 
        }); 
    }); 
    ...
    municipalityName().addEventListener('change', async (e) => { 
        ...
        if (selectedLake === "All Municipalities" ) { 
            ...
            const response = await jsonLoader('load_lakes', { 
                projectName: getState().currentProject, lakeName: 'all' 
            }); stopLoading(); 
            if (response.status === "error") { alert(response.message); return; } 
    ...
    municipalityName().addEventListener('click', async (e) => { 
        ...
        allLi.textContent = "All Municipalities"; 
        allLi.style.fontWeight = "bold"; 
        allLi.dataset.value = "All Municipalities"; 
        allLi.style.fontSize = "14px"; 
    ...
    municipalityName().addEventListener('input', (e) => { 
        ...
        if (value !== "") { 
            renderProjects(municipalityList(), municipalityName(), Object.keys(lakesData), value); 
        } else { 
            // lakeLayer = clearMap(lakeLayer, lakeMap); 
            lakeSelector().value = ""; municipalityName().value = ""; 
            lakeLabel().style.display = "none"; 
            lakeSelector().style.display = "none"; 
            municipalityName().dispatchEvent(new Event('click')); 
        } 
    }); 
    municipalityName().addEventListener('blur', () => { 
    ...
    const handleLakeSearchEvent = (e) => { 
        ...
        addItems(value); municipalityName().value = ''; 
        municipalityList().style.display = "none"; 
    }; 
    ...
    lakeSelector().addEventListener('change', async () => { 
        ...
        } else { menuContent().style.display = "flex"; } 
        ...
        const response = await jsonLoader('load_lakes', { 
            projectName: getState().currentProject, lakeName: lakeName 
        }); stopLoading(); 
        if (response.status === "error") { alert(response.message); return; } 
        dataLake = response.content.lake; dataDepth = response.content.depth; 
