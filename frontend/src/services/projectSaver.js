...
export async function saveProject(elements) {
    ...
    if (obsPoints.rows.length > 0) { 
        ...
        const content = {
            projectName: name, fileName: obsFileName, 
            data: obsPoints.rows, key: 'obs'
        };
        const output = await jsonLoader('save_obs', content);
    ...
    if (crossSections.rows.length > 0 && crossName !== '') {
        ...
        const content = {
            projectName: name, fileName: crossFileName, 
    ...
    const checkBoundary = await jsonLoader('check_condition', query);
    ...
    const queryHydro = {projectName: name, forceName: 'FlowFM_hyd.ext'};
    const checkHydro = await jsonLoader('check_condition', queryHydro);
